...
size_t from_pos = origin_val.find("from ");
    ...
    // 2. FIX: Use the 'species_count' map to track and increment the count
    //    (Requires: std::map<std::string, int> species_count; declared in main)
    species_count[prefix]++;

    int current_count = species_count[prefix];

    // 3. Format the count into a two-digit ID string (e.g., Hy01)
    std::stringstream ss;
